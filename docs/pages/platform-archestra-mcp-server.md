...
| `reload_mcp_server_tools` | Re-discover a deployed MCP server's tools from the live server and refresh Archestra's tool catalog for it — picks up added, removed, and changed tools (names, descriptions, and input schemas) with... | None (no additional RBAC permission required) |
...
| `bulk_remove_tools_from_agents` | Remove multiple tools from multiple agents in bulk. | None (no additional RBAC permission required) |
...
| `edit_skill` | Make a targeted edit to an existing Agent Skill without resending the whole SKILL.md. | None (no additional RBAC permission required) |
...
| Tool | Description | Required RBAC Permission |
|------|-------------|--------------------------|
| `memory` | View, search, create, update, or delete durable agent memory. | `memory:read` |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `command` | `"view" \| "create" \| "update" \| "search" \| "delete"` | Yes |  |
| `content` | `string` | No |  |
| `id` | `string` | No |  |
| `query` | `string` | No |  |
