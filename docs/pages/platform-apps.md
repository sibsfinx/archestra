...
Run an app full-page at `/a/:id` (no chat chrome, no sidebar), manage it (preview, tools, sharing, versions, settings) at `/apps/:id`, or run it from chat: a successful `scaffold_app`, `edit_app`, or `render_app` call renders the app inline in the conversation. All surfaces drive the same app-bound runtime, so behavior is identical.
